...
public class App extends AbstractVerticle {
	...
	public void start(Future<Void> startFuture) {
		...
		JsonObject config = new JsonObject().put("connection_string",
				"mongodb://localhost:27017").put("db_name", "cmad");
	...
	public void submitBlog(Router router) {
		router.post("/Services/rest/blogs").handler(
				ctx -> {

					JsonObject blogDetails = ctx.getBodyAsJson();
					client.findOne("blogs",
							new JsonObject().put("title",
									blogDetails.getString("title")),
							null,
							lookup -> {
							...
							if (blog != null) {
								...
								client.insert(
										...
										insert -> {
											...
											// add the generated id to the user
											// object
											blogDetails.put("_id",
													insert.result());
	...
	public void submitComments(Router router) {
		router.post("/Services/rest/blogs/:id/comments").handler(
				ctx -> {
					...
					client.findOne("blogs",
							new JsonObject().put("_id", blogId), null,
							lookup -> {
	...
	public void loginUser(Router router) {
		router.post("/Services/rest/user/auth").handler(
				ctx -> {
					...
					client.findOne(
							"users",
							new JsonObject().put("userName",
									userDetails.getString("userName")).put(
									"password",
									userDetails.getString("password")), null,
							lookup -> {
							...
							if (user != null) {
								...
								ctx.addCookie(Cookie.cookie("userName",
										userDetails.getString("userName")));
								ctx.response().putHeader(
										HttpHeaders.CONTENT_TYPE,
										"application/json");
	...
	public String insertSites(String companyId, String deptName,
			RoutingContext ctx) {

		String defaultSiteName = "London";

		JsonObject sitesObj = new JsonObject().put("companyId", companyId).put(
		...
		client.findOne("site", sitesObj, null, lookup -> {
			// error handling
				...
				JsonObject site = new JsonObject().put("companyId", companyId)
						.put("siteName", defaultSiteName);
				client.insert("site",
						site,
						insert -> {
							// error handling
						...
						System.out.println("Site insert result : "
								+ insert.result());
	...
	public String insertDept(String companyId, String siteId, String deptName,
			RoutingContext ctx) {

		JsonObject deptObj = new JsonObject().put("companyId", companyId).put(
				"siteId", siteId);
		client.findOne(
				"dept",
				deptObj,
				...
				lookup -> {
					...
					System.out.println("Dept Lookup result : "
							+ lookup.result());
					if (lookup.result() != null) {
						// already exists
						// do nothing
						...
						client.insert("dept",
								...
								System.out.println("Dept insert result : "
										+ insert.result());
								// add the generated id to the dept object
	...
	public void insertCompany(JsonObject userDetails, String deptName,
			RoutingContext ctx) {

		JsonObject companyObj = new JsonObject().put("companyName",
				userDetails.getString("companyName")).put("subdomain",
				userDetails.getString("subdomain"));
		client.findOne(
				"company",
				companyObj,
				null,
				lookup -> {
					...
					System.out.println("Company Lookup result : "
							+ lookup.result());
					...
					if (lookup.result() != null) {
						...
						JsonObject company = new JsonObject().put(
								"companyName",
								userDetails.getString("companyName"))
								.put("subdomain",
										userDetails.getString("subdomain"));
						client.insert("company",
								company,
								insert -> {
									// error handling
								if (insert.failed()) {
								...
								System.out.println("Company insert result : "
										+ insert.result());
								// add the generated id to the dept object
		...
		router.post("/Services/rest/user/register")
				.handler(
						ctx -> {
							...
							JsonObject userDetails = ctx.getBodyAsJson();
							Boolean isCompany = userDetails
									.getBoolean("isCompany");
							if (userDetails.containsKey("isCompany")
									&& isCompany) {
								vertx.executeBlocking(future -> {
									// Call some blocking API that takes a
									// significant amount of time to return
										this.insertCompany(userDetails,
												userDetails
														.getString("deptName"),
												ctx);
										future.complete();
	...
	public void getCompany(Router router) {
		router.get("/Services/rest/company").handler(
				ctx -> {
	...
	public void getSites(Router router) {
		router.get("/Services/rest/company/:id/sites").handler(
				ctx -> {

					client.find("site",
							new JsonObject().put("companyId", ctx.request()
									.getParam("id")),
							lookup -> {
								// error handling
							if (lookup.failed()) {
	...
	public void getDept(Router router) {
		router.get("/Services/rest/company/:id/sites/:siteid/departments")
				.handler(
						ctx -> {

							client.find(
									"dept",
									new JsonObject().put("siteId", ctx
	...
	public void getBlogs(Router router) {
		router.get("/Services/rest/blogs").handler(
				ctx -> {
	...
	public void searchBlogsWithTags(Router router) {
		router.get("/Services/rest/blogs/:tags").handler(
				ctx -> {

					client.find("blogs",
							new JsonObject().put("tags", ctx.request()
									.getParam("tags")),
							lookup -> {
								// error handling
							if (lookup.failed()) {
	...
	public static void main(String[] args) {
		...
		vertx.deployVerticle("com.cisco.cmad.vertx_backend.App",
